...
function Naim () {
  ...
  _getViewerCountCallbacks = [];
  this.getViewerCount = function (done) {
    _getViewerCountCallbacks.push(done);
    socket.write(getAudienceStatusPacket());    
  };
  ...
  function parseAIMPacket(packet) {
    ...
    //    console.info("parseAIMPacket", type);

    switch (type) {
        // response to user status call
        case EVENT_AUDIENCE_STATUS: 
          var count = packet[5];
          _getViewerCountCallbacks.shift()(null, count);
          break;
  ...
  this.connect = function(host, port, done) {
    ...
    socket = net.connect(
      { host:HOST, port:PORT }, done);
                         //        parseAIMPacket(d);
                         //        pollForViewers(); // do this elsewhere
